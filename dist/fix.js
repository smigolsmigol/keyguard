...
export async function runFix(projectRoot) {
    const fixed = [];
    const manual = [];
...
async function pinCIActions(projectRoot, fixed, manual) {
...
function addCIPermissions(projectRoot, fixed) {
    const workflowDir = join(projectRoot, '.github', 'workflows');
    if (!existsSync(workflowDir))
        return;
    let files;
...
function patchGitignore(projectRoot, fixed) {
...
function createIgnoreFile(projectRoot, filename, fixed) {
...
function createSecurityMd(projectRoot, fixed) {
...
export function formatFix(result) {
    const lines = [];
