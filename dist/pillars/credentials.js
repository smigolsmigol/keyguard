...
function patternCovered(pattern, lines) {
    for (const line of lines) {
        ...
        if (pattern === '.env.*' && (line === '.env*' || line === '.env.*'))
            return true;
        // *.key covers all .key files
        if (pattern.startsWith('*.') && line === pattern)
            return true;
    }
    return false;
}
...
function checkAiContextExclusion(projectRoot) {
    const findings = [];
