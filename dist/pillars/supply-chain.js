...
function parseActionRef(usesLine) {
...
function scanWorkflow(filePath, relPath, writeAllowed) {
    let content;
    try {
    ...
    const workflow = loadYamlString(content);
    if (!workflow)
        return [];
    const findings = [];
...
export async function scan(ctx) {
    ...
    const parts = [];
    if (unpinned > 0)
        parts.push(`${unpinned} unpinned action(s)`);
    if (missingPerms > 0)
        parts.push(`${missingPerms} workflow(s) missing permissions`);
