...
function checkSecurityMd(projectRoot: string): Finding | null {
...
function checkContributingMd(projectRoot: string): Finding | null {
...
function checkReadmeSecuritySection(projectRoot: string): Finding | null {
...
function checkPreCommitHooks(projectRoot: string): Finding | null {
...
export async function scan(ctx: ScanContext): Promise<PillarResult> {
  ...
  const status: 'pass' | 'warn' = hasMedium ? 'warn' : 'pass';
